  ...
  <div class="order">
    <div class="order-card-body" v-for="(order, index) in orderlist" :key="index">
      <div
        ...
        v-if="order.orderInfo.shopInfo"
      >
        <img :src="order.orderInfo.shopInfo.image_path" alt />
        <div class="order-card-content">
          <div class="order-card-head">
            <div class="title">
              <a @click.stop="$router.push('/goods')">
      ...
      <div class="order-card-bottom">
        <button class="cardbutton" @click="$router.push('/goods')">再来一单</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getData() {
      this.$axios
        .get(`/api/user/orders/${localStorage.ele_login}`)
        .then(res => {
